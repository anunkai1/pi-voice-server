...
/**
 * Keep capability and residency separate in the public health contract.
 * modelLoaded retains its original literal meaning; modelResident is the
 * clearer alias used by new consumers.
 */
export function modelHealthState({ resident, loading }) {
	return {
		modelAvailable: true,
		modelLoaded: Boolean(resident),
		modelResident: Boolean(resident),
		modelLoading: Boolean(loading),
	};
}
